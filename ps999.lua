...
-- AUTO FRUIT ĐÃ SỬA: Chỉ ăn 1 quả mỗi loại + cooldown 5 phút
task.spawn(function()
    local COOLDOWN = 300  -- 5 phút
    
    local lastEatTime = {}  -- [fruitName] = thời gian ăn lần cuối
    
    local function TryEatOne()
        local fruitInv = Save.Get().Inventory.Fruit or {}
        local activeFruits = FruitCmds.GetActiveFruits()
        
        for uid, data in pairs(fruitInv) do
            local fruitName = data.id
            if not fruitName or fruitName == "Candycane" then continue end
            
            -- Kiểm tra cooldown riêng từng loại
            if os.time() - (lastEatTime[fruitName] or 0) < COOLDOWN then continue end
            
            local current = activeFruits[fruitName] and #activeFruits[fruitName] or 0
            if current >= 20 then continue end
            
            -- Chỉ ăn đúng 1 quả
            pcall(function()
                Network.Invoke("Fruits: Consume", uid, 1)
                lastEatTime[fruitName] = os.time()
                UI:SetText("Status", "Ate 1x " .. fruitName)
            end)
            
            task.wait(0.6)  -- delay an toàn
        end
    end
    
    task.wait(5)           -- đợi script load xong
    TryEatOne()
    
    while true do
        TryEatOne()
        task.wait(60)      -- kiểm tra mỗi 1 phút
    end
end)
